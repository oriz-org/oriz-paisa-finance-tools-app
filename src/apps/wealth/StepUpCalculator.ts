...
export function render(): HTMLElement {
  ...
  function update(): void {
    ...
    stats.appendChild(
      createResultCard({ label: 'Total Invested', value: formatCurrency(result.investedAmount) })
    );
    stats.appendChild(
      createResultCard({
    ...
    stats.appendChild(
      createResultCard({ label: 'Wealth Gained', value: formatCurrency(result.wealthGained) })
    );
    ...
    chart.render({
      ...
      datasets: [
        {
          label: 'Value',
          data: result.yearlyBreakdown.map((y) => y.value),
          type: 'growth',
          fill: true,
        },
        {
          label: 'Invested',
          data: result.yearlyBreakdown.map((y) => y.invested),
          type: 'neutral',
          fill: true,
        },
      ],
    ...
    askAI(
      `Step-up SIP: ₹${formatIndianNumber(state.monthly)}/month with ${state.stepUp}% annual increase for ${state.years} years at ${state.rate}%. Total: ₹${formatIndianNumber(result.totalValue)}. 2 insights.`,
      'advisor'
    )
  ...
  step.style.marginTop = 'var(--space-6)';
  step.appendChild(
    createSmartInput({
      id: 'stepup',
