...
export function render(): HTMLElement {
  ...
  function update(): void {
    ...
    stats.appendChild(createResultCard({ label: 'CAGR', value: `${cagr}%`, accent: true, subtext: 'Annualized Return' }));
    stats.appendChild(createResultCard({ label: 'Total Growth', value: `${((state.final - state.initial) / state.initial * 100).toFixed(1)}%` }));
    results.appendChild(stats);
  }

  inputs.innerHTML = '<h3 style="margin-bottom: var(--space-4);">Values</h3>';
  inputs.appendChild(createSmartInput({ id: 'init', label: 'Initial Value', min: 1000, max: 10000000, value: state.initial, step: 10000, prefix: '₹', currency: true, onChange: (v) => { state.initial = v; update(); } }));
  const f = document.createElement('div'); f.style.marginTop = 'var(--space-6)';
  f.appendChild(createSmartInput({ id: 'final', label: 'Final Value', min: 1000, max: 100000000, value: state.final, step: 10000, prefix: '₹', currency: true, onChange: (v) => { state.final = v; update(); } }));
