...
export function render(): HTMLElement {
  ...
  function update(): void {
    ...
    stats.appendChild(
      createResultCard({ label: 'Principal Amount', value: formatCurrency(state.principal) })
    );
    stats.appendChild(
      createResultCard({
        label: 'Total Value',
    ...
    stats.appendChild(
      createResultCard({
        ...
        subtext: `${((result.wealthGained / state.principal) * 100).toFixed(1)}% returns`,
      })
    );
    ...
    chart.render({
      ...
      datasets: [
        ...
        {
          label: 'Principal',
          data: result.yearlyBreakdown.map(() => state.principal),
          type: 'neutral',
          fill: false,
        },
      ],
    ...
    askAI(
      `I invested ₹${formatIndianNumber(state.principal)} as lumpsum for ${state.years} years at ${state.rate}%. It will grow to ₹${formatIndianNumber(result.totalValue)}. Give 2 short insights.`,
      'advisor'
    )
