  ...
  function update(): void {
    ...
    stats.appendChild(
      createResultCard({
        ...
        subtext: `${result.interestPercent}% of total`,
      })
    );
    stats.appendChild(
      createResultCard({ label: 'Total Payment', value: formatCurrency(result.totalPayment) })
    );
    ...
    askAI(
      `Home loan: ₹${formatIndianNumber(state.principal)} at ${state.rate}% for ${state.tenure / 12} years. EMI: ₹${formatIndianNumber(result.emi)}. Interest: ₹${formatIndianNumber(result.totalInterest)}. Suggest prepayment strategy.`,
      'strategist'
    )
