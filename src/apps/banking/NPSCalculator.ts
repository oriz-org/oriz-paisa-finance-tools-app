...
export function render(): HTMLElement {
  ...
  function update(): void {
    ...
    stats.appendChild(createResultCard({ label: 'Total Corpus at 60', value: formatCurrency(result.totalCorpus), accent: true }));
    stats.appendChild(createResultCard({ label: 'Lumpsum (60%)', value: formatCurrency(result.lumpsum) }));
    stats.appendChild(createResultCard({ label: 'Monthly Pension', value: formatCurrency(result.monthlyPension), subtext: 'From 40% annuity' }));
    ...
    chart.render({
      ...
      datasets: [
        ...
        { label: 'Contributions', data: result.yearlyBreakdown.map((y) => y.contribution), type: 'neutral', fill: false },
      ],
    });
  }

  inputs.innerHTML = '<h3 style="margin-bottom: var(--space-4);">NPS Details</h3>';
  inputs.appendChild(createSmartInput({ id: 'mon', label: 'Monthly Contribution', min: 500, max: 100000, value: state.monthly, step: 500, prefix: '₹', currency: true, onChange: (v) => { state.monthly = v; update(); } }));
