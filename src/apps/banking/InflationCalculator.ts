...
export function render(): HTMLElement {
  ...
  function update(): void {
    ...
    if (state.reverse) {
      ...
      stats.appendChild(createResultCard({ label: `Future Value (in ${state.years} yrs)`, value: formatCurrency(state.amount) }));
      stats.appendChild(createResultCard({ label: "Today's Value", value: formatCurrency(present), accent: true }));
      stats.appendChild(createResultCard({ label: 'Loss in Value', value: formatCurrency(state.amount - present), subtext: `${(((state.amount - present) / state.amount) * 100).toFixed(1)}% decrease` }));
      results.appendChild(stats);
    } else {
      const future = calculateInflation(state.amount, state.rate, state.years);
      const stats = document.createElement('div'); stats.className = 'stats-grid';
      stats.appendChild(createResultCard({ label: "Today's Amount", value: formatCurrency(state.amount) }));
      stats.appendChild(createResultCard({ label: `Future Cost (in ${state.years} yrs)`, value: formatCurrency(future), accent: true }));
      stats.appendChild(createResultCard({ label: 'Increase', value: formatCurrency(future - state.amount), subtext: `${(((future - state.amount) / state.amount) * 100).toFixed(1)}% increase` }));
      results.appendChild(stats);
    }
  }
  ...
  t.appendChild(createToggle({ id: 'rev', label: 'Reverse (Future → Today)', checked: state.reverse, onChange: (v) => { state.reverse = v; update(); } }));
